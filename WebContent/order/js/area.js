...
var ZQNX = ZPCAData.split("#");

for (i = 0; i < ZQNX.length; i++) {

    ZQAreaArr[i] = [];

    TArea = ZQNX[i].split("$")[1].split("|");

    for (j = 0; j < TArea.length; j++) {

        ZQAreaArr[i][j] = TArea[j].split(",");

        if (ZQAreaArr[i][j].length == 1) ZQAreaArr[i][j][1] = ZArea;

        TArea[j] = TArea[j].split(",")[0]

    }

    ZAreaArr[i] = ZQNX[i].split("$")[0] + "," + TArea.join(",");

    ZProvinceArr[i] = ZAreaArr[i].split(",")[0];

    ZCityArr[i] = ZAreaArr[i].split(',')
...
function InitPCA() {
    ...
    this.SelP.onchange = function () {
        ...
        InitPCA.SetC(this.PCA)

    };

    if (this.SelA) this.SelC.onchange = function () {

        InitPCA.SetA(this.PCA)
...
InitPCA.SetP = function (PCA) {

    for (i = 0; i < ZProvinceArr.length; i++) {

        PCAPT = PCAPV = ZProvinceArr[i];

        if (PCAPT == ZProvince) PCAPV = "";
...
InitPCA.SetC = function (PCA) {

    PI = PCA.SelP.selectedIndex;

    PCA.SelC.length = 0;

    for (i = 1; i < ZCityArr[PI].length; i++) {

        PCACT = PCACV = ZCityArr[PI][i];

        if (PCACT == ZCity) PCACV = "";
    ...
    if (PCA.SelA) InitPCA.SetA(PCA)

};

InitPCA.SetA = function (PCA) {
    ...
    for (i = 1; i < ZQAreaArr[PI][CI].length; i++) {

        PCAAT = PCAAV = ZQAreaArr[PI][CI][i];

        if (PCAAT == ZArea) PCAAV = "";
